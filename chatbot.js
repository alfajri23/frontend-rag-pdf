(function(){const a="https://backend-rag-chatbot.ngeteh.online/chat";const t=`
    ...
    `;const n=document.createElement("style");n.textContent=t;document.head.appendChild(n);const e=document.createElement("div");e.id="my-chatbot-container";e.innerHTML=o;document.body.appendChild(e);const c=e.querySelector(".chatbot-toggle");const i=e.querySelector(".chat-close");const r=e.querySelector("#send-btn");const s=e.querySelector("#chat-input");const d=e.querySelector(".chat-body");const h=()=>{e.classList.toggle("open")};c.addEventListener("click",h);i.addEventListener("click",h);const l=()=>{const t=s.value.trim();if(t==="")return;p(t,"user");s.value="";const o=b();fetch(a,{method:"POST",headers:{t:"application/json",o:"application/json"},body:JSON.stringify({query:t})}).then(t=>{if(!t.ok){throw new Error(`HTTP error! status: ${t.status}`)}return t.json()}).then(t=>{o.remove();console.log("Response from backend:",t);p(t.response,"bot")})["catch"](t=>{console.error("Error fetching chatbot response:",t);o.remove();p("Maaf, terjadi kesalahan. Silakan coba lagi nanti.","bot")})};const p=(t,o)=>{const a=document.createElement("div");a.className=`message ${o}`;a.textContent=t;d.appendChild(a);d.scrollTop=d.scrollHeight};const b=()=>{const t=document.createElement("div");t.className="message bot loading";t.innerHTML="<span></span><span></span><span></span>";d.appendChild(t);d.scrollTop=d.scrollHeight;return t};r.addEventListener("click",l);s.addEventListener("keypress",t=>{if(t.key==="Enter"){t.preventDefault();l()}})})();